...
void AShooterGameCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindKey(EKeys::LeftShift, IE_Pressed, this, &AShooterGameCharacter::StartSprint);

	PlayerInputComponent->BindKey(EKeys::LeftShift, IE_Released, this, &AShooterGameCharacter::StopSprint);
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<AShooterGameProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
...
void AShooterGameCharacter::StartSprint()
{
	bOnSprint = true;
	...
	if (MovementComp)
	{
		MovementComp->MaxWalkSpeed *= 2.f;
	}
}

void AShooterGameCharacter::StopSprint()
{
	bOnSprint = false;
